...
export async function authenticateAdmin(config: GasConfig, username: string, password: string): Promise<GasResponse> {
  return sendToGasEndpoint(config.gasUrl, { action: 'authenticateAdmin', username, password });
}

export async function saveSettingsToGas(config: GasConfig, settings: any): Promise<GasResponse> {
  return sendToGasEndpoint(config.gasUrl, { action: 'saveSettings', sheetId: config.sheetId, settings: JSON.stringify(settings) });
}

export async function getSettingsFromGas(config: GasConfig): Promise<{settings?: string, success: boolean, error?: string}> {
  return sendToGasEndpoint(config.gasUrl, { action: 'getSettings', sheetId: config.sheetId });
}
...
export async function changeAdminPassword(
  ...
  return sendToGasEndpoint(config.gasUrl, { action: 'changeAdminPassword', oldPassword, newUsername, newPassword });
}
